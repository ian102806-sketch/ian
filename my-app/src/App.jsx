...
export default function App() {
  ...
  // --- UPDATED ADMIN FETCH (JOINS PROFILES) ---
              ...
              <div style={s.card}>
                ...
                <div style={{display: 'flex', gap: '16px', marginBottom: '24px', background: '#f8fafc', padding: '20px', borderRadius: '12px'}}>
                   ...
                   <div style={{flex: 2}}>
                      ...
                      <input style={{...s.input, marginBottom: 0}} placeholder="Search by email..." onChange={e => setUserSearch(e.target.value)} />
                   </div>
                </div>
                <div style={{overflowX: 'auto'}}>
                  <table style={{width: '100%', borderCollapse: 'collapse'}}>
                    ...
                    <tbody>
                      {allRecords.filter(r => (dateFilter === '' || r.date === dateFilter) && (userSearch === '' || (r.profiles?.email || '').toLowerCase().includes(userSearch.toLowerCase()))).map(rec => (
                        <tr key={rec.id}>
                          {/* SHOWING EMAIL HERE */}
                          <td style={{...s.td, fontWeight: '700', color: '#2563eb'}}>{rec.profiles?.email || 'N/A'}</td>
